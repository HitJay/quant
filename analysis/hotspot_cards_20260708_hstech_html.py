...
DAY_HUM = "2026-07-08"
TOPIC = "hstech_rally"
OUT = ROOT / f"output/hotspot/{DATE}/xhs_{TOPIC}_html_v2"
...
<div class="subtitle" style="font-size:34px">港股科技单日狂飙</div>
...
<div style="text-align:center;margin-top:36px">
  ...
  <div style="font-size:20px;color:var(--muted);margin-top:12px;letter-spacing:1px">
...
<div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:20px;margin-top:52px">
  <div class="card" style="text-align:center;padding:24px 12px">
    <div class="big-num" style="font-size:56px;color:var(--gold)">{HSTECH_AMT_YI}亿</div>
    <div style="font-size:16px;color:var(--muted);margin-top:12px">港主板成交</div>
  </div>
  <div class="card" style="text-align:center;padding:24px 12px">
    <div class="big-num" style="font-size:56px;color:var(--cyan)">+{NANXIANG_YI}亿</div>
    <div style="font-size:16px;color:var(--muted);margin-top:12px">南向净买</div>
  </div>
  <div class="card" style="text-align:center;padding:24px 12px">
    <div class="big-num" style="font-size:56px;color:var(--red)">+13.5%</div>
    <div style="font-size:16px;color:var(--muted);margin-top:12px">华虹领涨</div>
  </div>
</div>
...
<div style="margin-top:44px;padding:28px 32px;background:linear-gradient(135deg,var(--card) 0%,#1a1a1f 100%);border:2px solid var(--orange);border-radius:16px;box-shadow:0 0 32px rgba(210,153,29,0.15);text-align:center">
  <div style="font-size:20px;color:var(--text2);margin-bottom:14px">但 —— 历史上这个档位</div>
  <div class="big-num" style="font-size:72px;background:linear-gradient(90deg,var(--orange) 0%,var(--gold) 100%);-webkit-background-clip:text;-webkit-text-fill-color:transparent">20 日胜率仅 56%</div>
  <div style="font-size:16px;color:var(--muted);margin-top:14px">
    中位涨幅 +3.8%   ·   一步之遥就是 <b style="color:var(--red)">43%</b> 陷阱档
  </div>
</div>

<div style="margin-top:auto;text-align:center;padding-bottom:14px">
  <div style="display:inline-block;padding:16px 32px;background:var(--card);border:1.5px solid var(--cyan);border-radius:14px;font-size:22px;font-weight:700;color:var(--cyan);box-shadow:0 4px 16px rgba(86,212,221,0.15)">
    ⚠️ 追科技的姐妹们, 别急着 all in
  </div>
  <div style="font-size:15px;color:var(--muted);font-style:italic;margin-top:18px">翻到下一页 → 看今日港科到底涨了啥</div>
</div>
...
def page_2_html() -> str:
    ...
    <div class="hk-row">
      <div class="hk-name">
        ...
        <div style="font-size:13px;color:var(--dim);margin-top:3px">{code}</div>
      </div>
...
<div style="margin-top:auto;display:flex;flex-direction:column;gap:14px;padding-bottom:12px">
  <div style="text-align:center;padding:18px 24px;background:var(--card);border:1.5px solid var(--cyan);border-radius:12px;font-size:19px;font-weight:700;color:var(--cyan)">
    华虹 +13.5   ·   阿里 +13.0   ·   快手 +11.5  —  半导体 + 互联网双主线狂飙
  </div>
  <div style="display:flex;justify-content:space-around;text-align:center;font-size:16px;color:var(--muted)">
  ...
  <div style="text-align:center;font-size:17px;color:var(--gold);font-weight:600">
    ...
    <div class="etf-card">
      <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:12px">
        <div style="flex:1">
          ...
          <div style="font-size:13px;color:var(--dim);margin-top:4px">{code}</div>
          <div style="font-size:13px;color:var(--muted);margin-top:8px">{note}</div>
        </div>
        <div style="text-align:right">
          <div style="font-size:28px;font-weight:900;color:var({color})">+{pct:.2f}%</div>
          <div style="font-size:13px;color:var(--muted);margin-top:6px">{amt:.1f}亿</div>
        </div>
      </div>
    </div>"""
...
<div style="margin-top:24px;display:flex;flex-direction:column;gap:14px">
  <div style="padding:16px 22px;background:var(--card2);border-left:5px solid var(--red);border-radius:8px">
    <div style="font-size:18px;font-weight:700;color:var(--red);margin-bottom:6px">港科主题 ETF · 直接买港股</div>
    <div style="font-size:15px;color:var(--text2)">513010 是唯一沪市主流港科ETF, 成交 16 亿最活跃; 159120/159125 深市备份</div>
  </div>
  <div style="padding:16px 22px;background:var(--card2);border-left:5px solid var(--orange);border-radius:8px">
    <div style="font-size:18px;font-weight:700;color:var(--orange);margin-bottom:6px">A 股 AI 算力 · 蹭港科溢出</div>
    <div style="font-size:15px;color:var(--text2)">半导体设备 ETF +9.04% 一枝独秀, 华为 Atlas 950 催化 + 联动科技/华峰测控齐涨</div>
  </div>
</div>
...
<div style="margin-top:auto;padding:20px 24px;background:linear-gradient(135deg,rgba(86,212,221,0.08),rgba(86,212,221,0.02));border:1.5px solid var(--cyan);border-radius:12px;text-align:center;margin-bottom:10px">
  <div style="font-size:20px;font-weight:900;color:var(--cyan)">💡 想上车港科, 选 513010 最直接</div>
  <div style="font-size:14px;color:var(--orange);font-style:italic;margin-top:8px">港科溢价问题下页说, 别只看涨幅冲进去</div>
</div>
...
def page_4_html() -> str:
    ...
    for label, n, w5, w20, m20, med20, m60, med60, is_current in BUCKETS:
        ...
        hl_line = f'<div style="font-size:15px;font-weight:900;color:var(--orange);margin-top:10px">← 今日 +{HSTECH_PCT}% 在此档</div>' if is_current else ""
        bucket_cards.append(f"""
<div class="{cls}">
  <div style="display:grid;grid-template-columns:210px 1fr 1fr 1fr;align-items:center;gap:22px">
    <div>
      <div style="font-size:32px;font-weight:900;color:var(--red)">{label}</div>
      <div style="font-size:15px;color:var(--muted);margin-top:8px">样本 n = {n}</div>
      {hl_line}
    </div>
    <div style="text-align:center">
      <div style="font-size:14px;color:var(--muted);margin-bottom:8px">5 日胜率</div>
      <div style="font-size:44px;font-weight:900;color:var({w5_col})">{w5}%</div>
    </div>
    <div style="text-align:center">
      <div style="font-size:14px;color:var(--muted);margin-bottom:8px">20 日胜率</div>
      <div style="font-size:44px;font-weight:900;color:var({w20_col})">{w20}%</div>
      <div style="font-size:13px;color:var(--muted);margin-top:6px">均 {m20:+.1f}%</div>
    </div>
    <div style="text-align:center">
      <div style="font-size:14px;color:var(--muted);margin-bottom:8px">60 日均值</div>
      <div style="font-size:44px;font-weight:900;color:var({m60_col})">{m60:+.1f}%</div>
      <div style="font-size:13px;color:var(--muted);margin-top:6px">中位 {med60:+.1f}%</div>
    </div>
  </div>
</div>""")
...
<div style="margin-top:26px;text-align:center">
  <div style="font-size:20px;font-weight:900;color:var(--text);margin-bottom:12px">🔑 关键发现</div>
  <div style="font-size:19px;color:var(--cyan);margin-bottom:22px">
...
<div style="margin-top:auto;padding:22px 28px;background:linear-gradient(135deg,var(--card) 0%,rgba(210,153,29,0.08) 100%);border:2px solid var(--orange);border-radius:14px;text-align:center;box-shadow:0 0 28px rgba(210,153,29,0.15);margin-bottom:10px">
  <div style="font-size:22px;font-weight:900;color:var(--orange)">⚠️ 明天再涨半根阳线, 跨进 [6%, 7%) 档</div>
  <div style="font-size:16px;color:var(--muted);margin-top:12px">20 日胜率 <b style="color:var(--red)">56%</b> → <b style="color:var(--green)">43%</b>, 60 日均 <b style="color:var(--green)">-7%</b> (翻到下一页 →)</div>
</div>
...
<div class="subtitle" style="font-size:32px">为什么 [6%, 7%) 是死亡档?</div>

<div style="display:grid;grid-template-columns:1fr auto 1fr;align-items:center;gap:24px;margin-top:32px;padding:0 20px">
  <div style="text-align:center">
    <div style="font-size:20px;color:var(--muted);margin-bottom:10px">[5%, 6%)</div>
    <div class="big-num" style="font-size:170px;color:var(--red);filter:drop-shadow(0 4px 12px rgba(248,81,73,0.3))">56%</div>
    <div style="font-size:16px;color:var(--muted);margin-top:8px">20 日胜率</div>
  </div>
  <div style="text-align:center;padding:0 16px">
    <div style="font-size:110px;font-weight:900;color:var(--orange);line-height:1">→</div>
    <div style="font-size:15px;color:var(--orange);margin-top:8px;font-weight:700">跨半根阳线</div>
  </div>
  <div style="text-align:center">
    <div style="font-size:20px;color:var(--muted);margin-bottom:10px">[6%, 7%)</div>
    <div class="big-num" style="font-size:170px;color:var(--green);filter:drop-shadow(0 4px 12px rgba(63,185,80,0.3))">43%</div>
    <div style="font-size:16px;color:var(--muted);margin-top:8px">20 日胜率</div>
  </div>
</div>

<div style="text-align:center;padding:18px 28px;background:var(--card2);border:1px solid var(--border);border-radius:12px;margin-top:24px;font-size:20px;font-weight:700">
  60 日累计:  [5-6%) 均 <span style="color:var(--muted)">-0.9%</span>   VS   [6-7%) 均 <span style="color:var(--green)">-7.0%</span>
</div>

<div style="margin-top:24px">
  <div style="text-align:center;font-size:19px;font-weight:700;color:var(--cyan);margin-bottom:4px">历史相似形态: 短期连续两次 +5%</div>
  <div style="text-align:center;font-size:14px;color:var(--muted);font-style:italic;margin-bottom:14px">20 日后表现 · 最近 8 次</div>
...
<div style="margin-top:auto;padding:24px 32px;background:linear-gradient(135deg,var(--orange) 0%,#c48819 100%);border-radius:14px;text-align:center;margin-bottom:10px;box-shadow:0 6px 24px rgba(210,153,29,0.3)">
  <div style="font-size:28px;font-weight:900;color:var(--bg)">⚠️ 12 次里 5 次亏钱, 4 次跌破 -3%</div>
  <div style="font-size:16px;color:var(--bg);margin-top:10px;opacity:0.85">20 日均 +2.94%   ·   中位 +6.04%   ·   但"高开低走"是最大风险</div>
</div>
...
def page_6_html() -> str:
    ...
    <div style="display:flex;align-items:center;gap:20px;padding:16px 20px;background:var(--card);border:1px solid var(--border);border-radius:12px">
      <div style="min-width:88px;padding:12px 20px;background:var({col});color:var(--bg);border-radius:22px;font-size:19px;font-weight:900;text-align:center">{tag}</div>
      <div style="font-size:17px;color:var(--text2);line-height:1.5;flex:1">{body}</div>
...
<div style="text-align:center;margin-top:32px">
  <div style="font-size:18px;color:var(--muted);margin-bottom:12px">近 1 年分位</div>
  <div class="big-num" style="font-size:200px;background:linear-gradient(180deg,#5dd469 0%,#3fb950 100%);-webkit-background-clip:text;-webkit-text-fill-color:transparent;filter:drop-shadow(0 4px 12px rgba(63,185,80,0.3))">{POS_1Y:.1f}%</div>
  <div style="font-size:18px;color:var(--cyan);margin-top:14px;font-weight:600">低位区 · 反弹初期给博弈缓冲</div>
</div>
...
<div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:18px;margin-top:34px">
  <div class="card" style="text-align:center;padding:20px 12px">
    <div style="font-size:14px;color:var(--muted);margin-bottom:10px">距 1 年高点</div>
    <div style="font-size:44px;font-weight:900;color:var(--green)">{DIST_HIGH_1Y:+.1f}%</div>
    <div style="font-size:13px;color:var(--muted);margin-top:8px">{HIGH_1Y} → {HSTECH_CLOSE:.0f}</div>
  </div>
  <div class="card" style="text-align:center;padding:20px 12px">
    <div style="font-size:14px;color:var(--muted);margin-bottom:10px">距 1 年低点</div>
    <div style="font-size:44px;font-weight:900;color:var(--red)">+{DIST_LOW_1Y:.1f}%</div>
    <div style="font-size:13px;color:var(--muted);margin-top:8px">{LOW_1Y} → {HSTECH_CLOSE:.0f}</div>
  </div>
  <div class="card" style="text-align:center;padding:20px 12px">
    <div style="font-size:14px;color:var(--muted);margin-bottom:10px">南向近 1 年分位</div>
    <div style="font-size:44px;font-weight:900;color:var(--red)">{NANXIANG_1Y_RANK:.0f}%</div>
    <div style="font-size:13px;color:var(--muted);margin-top:8px">+130亿 · 日均+42亿</div>
  </div>
</div>

<div style="margin-top:32px">
  <div style="text-align:center;font-size:22px;font-weight:900;color:var(--text);margin-bottom:14px">🎯 三档操作建议</div>
...
<div style="margin-top:auto;padding:18px 24px;background:linear-gradient(90deg,rgba(210,153,29,0.12),rgba(210,153,29,0.03));border-left:5px solid var(--orange);border-radius:8px;margin-bottom:10px">
  <div style="font-size:18px;font-weight:900;color:var(--orange)">⚠️ 散户友情提醒</div>
  <div style="font-size:14px;color:var(--text2);margin-top:8px">港股 ≠ A 股 · 无涨跌幅限制 · 汇率波动风险 · 高位股次日常见 -3% 补跌</div>
</div>
...
<div style="display:flex;align-items:center;gap:24px;padding:26px 28px;background:linear-gradient(135deg,var(--card) 0%,rgba(0,0,0,0.3) 100%);border:2px solid var({col});border-radius:16px;box-shadow:0 4px 20px rgba(0,0,0,0.3)">
  <div style="font-size:64px;font-weight:900;color:var({col});min-width:96px;text-align:center;line-height:1;filter:drop-shadow(0 2px 8px rgba(0,0,0,0.4))">{num}</div>
  <div style="flex:1">
    <div style="font-size:26px;font-weight:900;color:var(--text)">{title}</div>
    <div style="font-size:16px;color:var(--muted);margin-top:8px">{body}</div>
  </div>
</div>""" for num, title, col, body in cards)

    body = f"""
<div class="top-pill"><div class="pill" style="background:var(--rose)">关注我</div></div>
<div style="text-align:center;font-size:20px;font-style:italic;color:var(--text2);margin-top:20px">
  港科明天还能续命吗? 数据每天替你盯 📊
</div>

<div style="text-align:center;margin-top:36px">
  <div style="font-size:46px;color:var(--text);margin-bottom:14px">每天 3 分钟</div>
  <div class="big-num" style="font-size:70px;background:linear-gradient(90deg,var(--gold) 0%,var(--gold2) 100%);-webkit-background-clip:text;-webkit-text-fill-color:transparent;filter:drop-shadow(0 4px 12px rgba(240,192,64,0.3))">看懂 A 股 + 港股</div>
</div>
...
<div style="margin-top:32px;padding:28px 32px;background:linear-gradient(135deg,var(--gold) 0%,#e8b73a 100%);border-radius:16px;text-align:center;box-shadow:0 8px 32px rgba(240,192,64,0.35)">
  <div style="font-size:32px;font-weight:900;color:var(--bg)">🔔 点关注 + 收藏 不迷路</div>
  <div style="font-size:17px;color:var(--bg);margin-top:10px;opacity:0.8">明早 9:15 继续给你递港科盘前情报</div>
</div>

<div style="margin-top:auto;text-align:center;padding-bottom:12px">
  <div style="font-size:19px;font-weight:900;color:var(--cyan);margin-bottom:10px">💬 评论区告诉我</div>
  <div style="font-size:17px;color:var(--text2);margin-bottom:8px">你是 港科党 还是 A股党? 明天该抄底还是减仓?</div>
  <div style="font-size:14px;color:var(--muted)">明天想看 哪只港股 or ETF 的盘后追踪? 评论区点名 →</div>
</div>
