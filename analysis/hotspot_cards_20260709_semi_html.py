...
<div class="subtitle" style="font-size:44px">半导体全线爆发 % 今日主角</div>
<div style="text-align:center;margin-top:24px">
  ...
  <div style="font-size:30px;color:var(--muted);margin-top:6px;letter-spacing:1px">半导体 % 申万二级 % 近 3 年第 1.2% 分位</div>
</div>
...
<div style="padding:20px 24px;background:linear-gradient(135deg,var(--card) 0%,#1a1a1f 100%);border:2px solid var(--orange);border-radius:12px;box-shadow:0 0 20px rgba(210,153,29,.15);text-align:center">
  <div style="font-size:28px;color:var(--text2);margin-bottom:8px">但历史数据显示</div>
  <div class="big-num" style="font-size:72px;background:linear-gradient(90deg,var(--orange) 0%,var(--gold) 100%);-webkit-background-clip:text;-webkit-text-fill-color:transparent">[6%,7%) 档 20d 胜率 {CROSS_PCT}%</div>
  <div style="font-size:28px;color:var(--muted);margin-top:8px">[5%,6%) 档反而是 <b style="color:var(--red)">65%</b> ! 多涨半根阳线反而胜率更低?</div>
</div>
<div style="text-align:center;margin-top:10px">
  <div style="display:inline-block;padding:12px 24px;background:var(--card);border:1.5px solid var(--cyan);border-radius:12px;font-size:30px;font-weight:700;color:var(--cyan);box-shadow:0 4px 14px rgba(86,212,221,.15)">半导体的反直觉规律</div>
...
def page_2_html() -> str:
    ...
    max_pct_ref = max_pct * 1.15  # 留头部空间让条形长短有区分, 但不压太短
    rows = []
    for name, price, pct_chg, vol, tag in STOCKS:
        ...
        vol_str = f"{vol:.0f}亿"
        rows.append(f"""<div style="display:grid;grid-template-columns:90px 80px 1fr 110px 100px 80px;align-items:center;gap:8px;padding:9px 12px;border-bottom:1px solid var(--border)">
<div style="font-size:30px;color:var(--text2);font-weight:600;white-space:nowrap">{name}</div>
<div style="font-size:26px;color:var(--muted);text-align:right">{price:.2f}</div>
<div style="height:24px;display:flex;align-items:center"><div style="width:{bar_w};height:20px;background:linear-gradient(90deg,var({bc}) 0%,rgba(248,81,73,.25) 100%);border-radius:3px;min-width:8px"></div></div>
...
<div style="padding:3px 12px;border-radius:10px;font-size:22px;font-weight:700;text-align:center;background:{'var(--red)' if tag=='涨停' else 'var(--orange)' if tag=='强势' else 'var(--muted)'};color:var(--bg)">{tag}</div></div>""")
    body = f"""<div class="top-pill"><div class="pill" style="background:var(--blue)">今日龙头</div></div>
<div class="subtitle">12 只半导体龙头放量涨停</div>
<div class="subtitle-sm">涨停 7 只 · 主力净入 318.7 亿 · 东财人气霸榜 7/10</div>
<div style="flex:1;display:flex;flex-direction:column">
<div style="margin-top:18px;background:var(--card);border:1px solid var(--border);border-radius:10px;overflow:hidden">
<div style="display:grid;grid-template-columns:90px 80px 1fr 110px 100px 80px;align-items:center;gap:8px;padding:8px 12px;font-size:24px;font-weight:700;color:var(--muted);background:var(--card2);border-bottom:1.5px solid var(--border)">
<div>名称</div><div style="text-align:right">价格</div><div style="text-align:center">今日涨幅条</div><div style="text-align:right">涨幅</div><div style="text-align:right">成交额</div><div style="text-align:center">标签</div></div>{"".join(rows)}</div>
<div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:12px;margin-top:16px">
<div class="card" style="text-align:center;padding:18px 12px"><div class="c-label" style="font-size:24px;color:var(--muted)">涨停家数</div><div class="c-val" style="font-size:64px;color:var(--red)">13</div><div style="font-size:22px;color:var(--muted);margin-top:2px">行业 NO.1</div></div>
<div class="card" style="text-align:center;padding:18px 12px"><div class="c-label" style="font-size:24px;color:var(--muted)">主力净入</div><div class="c-val" style="font-size:64px;color:var(--red)">318.7亿</div><div style="font-size:22px;color:var(--muted);margin-top:2px">全行业 NO.1</div></div>
<div class="card" style="text-align:center;padding:18px 12px"><div class="c-label" style="font-size:24px;color:var(--muted)">东财人气</div><div class="c-val" style="font-size:64px;color:var(--gold)">7/10</div><div style="font-size:22px;color:var(--muted);margin-top:2px">半导体霸榜</div></div>
</div>
</div>
...
<div style="font-size:22px;color:var(--muted)">{code}</div>
<div style="font-size:30px;font-weight:700;color:var(--text);margin:4px 0">{name}</div>
<div class="big-num" style="font-size:44px;color:var(--red)">+{pct:.2f}%</div>
<div style="font-size:24px;color:var(--muted);margin-top:2px">{price:.3f}元</div>
</div>""")
...
<div style="font-size:25px;color:var(--text2);font-weight:500">{c[0]}</div>
<div style="font-size:27px;font-weight:900;color:var(--red)">+{c[1]}</div>
<div style="font-size:22px;color:var(--muted)">{c[2]}领涨</div>
...
<div style="margin-top:28px">
  <div style="font-size:27px;color:var(--muted);font-weight:600;margin-bottom:6px;padding-left:2px">概念涨幅 TOP5</div>
...
<div style="font-size:24px;color:var(--muted);margin-bottom:6px">半导体涨停占全天</div>
<div class="big-num" style="font-size:60px;color:var(--red)">17%</div>
<div style="font-size:22px;color:var(--muted);margin-top:4px">75 只涨停中 13 只</div>
</div>
<div style="text-align:center;padding:16px;background:var(--card);border:1px solid var(--border);border-radius:12px">
<div style="font-size:24px;color:var(--muted);margin-bottom:6px">电子板块总主力净入</div>
<div class="big-num" style="font-size:60px;color:var(--gold)">438.8亿</div>
<div style="font-size:22px;color:var(--muted);margin-top:4px">全行业第一</div>
</div>
</div>
</div>
...
<div style="font-size:26px;color:var(--text2);line-height:1.4">[5%,6%) 档 20d 胜率 <b style="color:var(--red)">65%</b> % 今日 [6%,7%) 仅 <b style="color:var(--orange)">55%</b><br>7%+ 暴涨反而 67% — [6%,7%) 是最尴尬中档</div>
</div>
...
<div style="font-size:32px;font-weight:900;color:var(--orange)">反直觉规律</div>
<div style="font-size:27px;color:var(--muted);margin-top:6px">最强是 [5%,6%) 和 [7%+), 中间档反而弱 — 看结构不看涨幅 (翻到下一页)</div>
</div>
...
<div style="text-align:right;font-size:{'23px' if neg else '19px'};font-weight:{'900' if neg else '500'};color:var(--green if neg else --red)">{r20}</div>
    ...
    extra = ".srow{display:grid;grid-template-columns:110px 110px 1fr 100px;gap:6px;padding:3px 10px;align-items:center;font-size:26px;color:var(--text2);border-radius:4px;font-variant-numeric:tabular-nums}"
    body = f"""<div class="top-pill"><div class="pill" style="background:var(--orange)">反共识陷阱</div></div>
<div class="subtitle" style="font-size:52px">换挡不如换结构</div>
<div style="display:grid;grid-template-columns:1fr auto 1fr;align-items:center;gap:14px;margin-top:20px;padding:0 14px">
<div style="text-align:center"><div style="font-size:26px;color:var(--muted);margin-bottom:2px">今日档 [6%,7%)</div><div class="big-num" style="font-size:130px;color:var(--red);filter:drop-shadow(0 4px 10px rgba(248,81,73,.3))">{CROSS_PCT}%</div><div style="font-size:27px;color:var(--muted);margin-top:2px">20d 胜率</div></div>
<div style="text-align:center;padding:0 4px"><div style="font-size:64px;font-weight:900;color:var(--orange);line-height:1">-></div><div style="font-size:25px;color:var(--orange);margin-top:2px;font-weight:700">若 5 日内<br>两次 +5%</div></div>
<div style="text-align:center"><div style="font-size:26px;color:var(--muted);margin-bottom:2px">双次大涨后</div><div class="big-num" style="font-size:130px;color:var(--green);filter:drop-shadow(0 4px 10px rgba(63,185,80,.3))">{CROSS_TRAP}%</div><div style="font-size:27px;color:var(--muted);margin-top:2px">20d 胜率</div></div>
</div>
<div style="text-align:center;padding:8px 14px;background:var(--card2);border:1px solid var(--border);border-radius:10px;margin-top:6px;font-size:26px;font-weight:700">60d 均值: 单次大涨 +1.6% &nbsp;VS&nbsp; 双次大涨后 <span style="color:var(--green)">{CROSS_60D:+.1f}%</span></div>
<div style="margin-top:10px"><div style="text-align:center;font-size:27px;font-weight:700;color:var(--cyan);margin-bottom:1px">追过第二次的姐妹最后怎样了?</div>
<div style="text-align:center;font-size:24px;color:var(--muted);font-style:italic;margin-bottom:2px">最近 4 次</div>
<div style="display:grid;grid-template-columns:110px 110px 1fr 100px;gap:6px;padding:3px 10px;font-size:25px;font-weight:900;color:var(--muted);border-bottom:1px solid var(--border);margin-bottom:2px"><div>首日</div><div>次日</div><div style="text-align:center">两次涨幅</div><div style="text-align:right">20d 后</div></div>
{"".join(sr)}</div>
...
<div style="font-size:36px;font-weight:900;color:var(--bg)">{FAIL_N} 次双涨中 {FAIL_LOSS} 次亏钱 % {FAIL_DRAWDOWN} 次跌超 -3%</div>
<div style="font-size:27px;color:var(--bg);margin-top:4px;opacity:.85">20d 均 -3.0% % 中位 -2.9%</div>
</div>
...
<div style="font-size:25px;color:var(--muted);margin-bottom:6px">近 3 年分位</div>
...
<div style="font-size:26px;color:var(--orange);margin-top:8px;font-weight:600">高位区 % 距 3 年高点仅 -{abs(DIST_HIGH):.0f}%</div>
</div>
...
<div class="card" style="text-align:center;padding:12px 8px"><div style="font-size:25px;color:var(--muted);margin-bottom:4px">距 3 年低点</div><div style="font-size:46px;font-weight:900;color:var(--red)">+{DIST_LOW:.0f}%</div><div style="font-size:22px;color:var(--muted);margin-top:2px">{LOW_3Y} -> 现值</div></div>
<div class="card" style="text-align:center;padding:12px 8px"><div style="font-size:25px;color:var(--muted);margin-bottom:4px">主力净入</div><div style="font-size:46px;font-weight:900;color:var(--red)">318.7亿</div><div style="font-size:22px;color:var(--muted);margin-top:2px">单日 NO.1</div></div>
</div>
...
<div style="padding:12px 18px;background:linear-gradient(90deg,rgba(210,153,29,.12),rgba(210,153,29,.03));border-left:4px solid var(--orange);border-radius:8px"><div style="font-size:25px;font-weight:900;color:var(--orange)">散户提醒</div><div style="font-size:26px;color:var(--text2);margin-top:2px">3 年分位 {POS_3Y}% 已是高位区, [6%,7%) 档 20d 胜率仅 55%. 追涨前先看位置.</div></div>
...
<div style="text-align:center;font-size:28px;font-style:italic;color:var(--text2);margin-top:14px">明天半导体还能续命吗? 数据每天替你盯</div>
<div style="text-align:center"><div style="font-size:46px;color:var(--text);margin-bottom:8px">每天 3 分钟</div><div class="big-num" style="font-size:80px;background:linear-gradient(90deg,var(--gold) 0%,var(--gold2) 100%);-webkit-background-clip:text;-webkit-text-fill-color:transparent;filter:drop-shadow(0 4px 10px rgba(240,192,64,.3))">看懂 A 股</div></div>
<div style="display:flex;flex-direction:column;gap:10px">{cards}</div>
<div style="padding:20px 26px;background:linear-gradient(135deg,var(--gold) 0%,#e8b73a 100%);border-radius:14px;text-align:center;box-shadow:0 8px 28px rgba(240,192,64,.35)"><div style="font-size:40px;font-weight:900;color:var(--bg)">点关注 + 收藏 不迷路</div><div style="font-size:28px;color:var(--bg);margin-top:6px;opacity:.8">明早 9:15 继续给你递盘前情报</div></div>
<div style="text-align:center"><div style="font-size:28px;font-weight:900;color:var(--cyan);margin-bottom:6px">评论区告诉我</div><div style="font-size:30px;color:var(--text2);margin-bottom:4px">半导体今天你追了吗? 明天减仓还是持有?</div><div style="font-size:25px;color:var(--muted)">明天想看哪只票的追踪? 评论区点名</div></div>
