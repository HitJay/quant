...
DAY_HUM = "2026-07-08"
TOPIC = "hstech_rally"
OUT = ROOT / f"output/hotspot/{DATE}/xhs_{TOPIC}_html_v5"
...
<div style="text-align:center;margin-top:36px">
  ...
  <div style="font-size:32px;color:var(--muted);margin-top:12px;letter-spacing:1px">
...
<div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:20px;margin-top:52px">
  <div class="card" style="text-align:center;padding:24px 12px">
    ...
    <div style="font-size:26px;color:var(--muted);margin-top:12px">港主板成交</div>
  </div>
  <div class="card" style="text-align:center;padding:24px 12px">
    <div class="big-num" style="font-size:68px;color:var(--cyan)">+{NANXIANG_YI}亿</div>
    <div style="font-size:26px;color:var(--muted);margin-top:12px">南向净买</div>
  </div>
  <div class="card" style="text-align:center;padding:24px 12px">
    <div class="big-num" style="font-size:68px;color:var(--red)">+13.5%</div>
    <div style="font-size:26px;color:var(--muted);margin-top:12px">华虹领涨</div>
  </div>
</div>
...
<div style="margin-top:44px;padding:28px 32px;background:linear-gradient(135deg,var(--card) 0%,#1a1a1f 100%);border:2px solid var(--orange);border-radius:16px;box-shadow:0 0 32px rgba(210,153,29,0.15);text-align:center">
  <div style="font-size:32px;color:var(--text2);margin-bottom:14px">但 —— 历史上这个档位</div>
  ...
  <div style="font-size:26px;color:var(--muted);margin-top:14px">
...
<div style="margin-top:auto;text-align:center;padding-bottom:14px">
  ...
  <div style="font-size:25px;color:var(--muted);font-style:italic;margin-top:18px">翻到下一页 → 看今日港科到底涨了啥</div>
</div>
...
def page_2_html() -> str:
    ...
    <div class="hk-row">
      <div class="hk-name">
        <div style="font-size:32px;font-weight:700;color:var(--text)">{name}</div>
        <div style="font-size:22px;color:var(--dim);margin-top:3px">{code}</div>
      </div>
...
<div style="margin-top:auto;display:flex;flex-direction:column;gap:14px;padding-bottom:12px">
  <div style="text-align:center;padding:18px 24px;background:var(--card);border:1.5px solid var(--cyan);border-radius:12px;font-size:30px;font-weight:700;color:var(--cyan)">
    华虹 +13.5   ·   阿里 +13.0   ·   快手 +11.5  —  半导体 + 互联网双主线狂飙
  </div>
  <div style="display:flex;justify-content:space-around;text-align:center;font-size:26px;color:var(--muted)">
  ...
  <div style="text-align:center;font-size:27px;color:var(--gold);font-weight:600">
...
def page_3_html() -> str:
    ...
    <div class="etf-card">
      <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:12px">
        <div style="flex:1">
          <div style="font-size:28px;font-weight:700;color:var(--text)">{name}</div>
          <div style="font-size:22px;color:var(--dim);margin-top:4px">{code}</div>
          <div style="font-size:22px;color:var(--muted);margin-top:8px">{note}</div>
        </div>
        <div style="text-align:right">
          <div style="font-size:36px;font-weight:900;color:var({color})">+{pct:.2f}%</div>
          <div style="font-size:22px;color:var(--muted);margin-top:6px">{amt:.1f}亿</div>
        </div>
      </div>
    </div>"""
...
<div style="margin-top:24px;display:flex;flex-direction:column;gap:14px">
  <div style="padding:16px 22px;background:var(--card2);border-left:5px solid var(--red);border-radius:8px">
    <div style="font-size:28px;font-weight:700;color:var(--red);margin-bottom:6px">港科主题 ETF · 直接买港股</div>
    <div style="font-size:25px;color:var(--text2)">513010 是唯一沪市主流港科ETF, 成交 16 亿最活跃; 159120/159125 深市备份</div>
  </div>
  <div style="padding:16px 22px;background:var(--card2);border-left:5px solid var(--orange);border-radius:8px">
    <div style="font-size:28px;font-weight:700;color:var(--orange);margin-bottom:6px">A 股 AI 算力 · 蹭港科溢出</div>
    <div style="font-size:25px;color:var(--text2)">半导体设备 ETF +9.04% 一枝独秀, 华为 Atlas 950 催化 + 联动科技/华峰测控齐涨</div>
  </div>
</div>
...
<div style="margin-top:auto;padding:20px 24px;background:linear-gradient(135deg,rgba(86,212,221,0.08),rgba(86,212,221,0.02));border:1.5px solid var(--cyan);border-radius:12px;text-align:center;margin-bottom:10px">
  <div style="font-size:32px;font-weight:900;color:var(--cyan)">💡 想上车港科, 选 513010 最直接</div>
  <div style="font-size:23px;color:var(--orange);font-style:italic;margin-top:8px">港科溢价问题下页说, 别只看涨幅冲进去</div>
</div>
...
def page_4_html() -> str:
    ...
    for label, n, w5, w20, m20, med20, m60, med60, is_current in BUCKETS:
        ...
        hl_line = f'<div style="font-size:25px;font-weight:900;color:var(--orange);margin-top:10px">← 今日 +{HSTECH_PCT}% 在此档</div>' if is_current else ""
...
<div style="margin-top:26px;text-align:center">
  <div style="font-size:32px;font-weight:900;color:var(--text);margin-bottom:12px">🔑 关键发现</div>
  <div style="font-size:30px;color:var(--cyan);margin-bottom:22px">
...
<div style="margin-top:auto;padding:22px 28px;background:linear-gradient(135deg,var(--card) 0%,rgba(210,153,29,0.08) 100%);border:2px solid var(--orange);border-radius:14px;text-align:center;box-shadow:0 0 28px rgba(210,153,29,0.15);margin-bottom:10px">
  ...
  <div style="font-size:26px;color:var(--muted);margin-top:12px">20 日胜率 <b style="color:var(--red)">56%</b> → <b style="color:var(--green)">43%</b>, 60 日均 <b style="color:var(--green)">-7%</b> (翻到下一页 →)</div>
</div>
...
def page_6_html() -> str:
    ...
    <div style="display:flex;align-items:center;gap:20px;padding:16px 20px;background:var(--card);border:1px solid var(--border);border-radius:12px">
      <div style="min-width:88px;padding:12px 20px;background:var({col});color:var(--bg);border-radius:22px;font-size:30px;font-weight:900;text-align:center">{tag}</div>
      <div style="font-size:27px;color:var(--text2);line-height:1.5;flex:1">{body}</div>
...
<div style="text-align:center;margin-top:32px">
  <div style="font-size:28px;color:var(--muted);margin-bottom:12px">近 1 年分位</div>
  ...
  <div style="font-size:28px;color:var(--cyan);margin-top:14px;font-weight:600">低位区 · 反弹初期给博弈缓冲</div>
</div>
...
<div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:18px;margin-top:34px">
  <div class="card" style="text-align:center;padding:20px 12px">
    <div style="font-size:23px;color:var(--muted);margin-bottom:10px">距 1 年高点</div>
    <div style="font-size:56px;font-weight:900;color:var(--green)">{DIST_HIGH_1Y:+.1f}%</div>
    <div style="font-size:22px;color:var(--muted);margin-top:8px">{HIGH_1Y} → {HSTECH_CLOSE:.0f}</div>
  </div>
  <div class="card" style="text-align:center;padding:20px 12px">
    <div style="font-size:23px;color:var(--muted);margin-bottom:10px">距 1 年低点</div>
    <div style="font-size:56px;font-weight:900;color:var(--red)">+{DIST_LOW_1Y:.1f}%</div>
    <div style="font-size:22px;color:var(--muted);margin-top:8px">{LOW_1Y} → {HSTECH_CLOSE:.0f}</div>
  </div>
  <div class="card" style="text-align:center;padding:20px 12px">
    <div style="font-size:23px;color:var(--muted);margin-bottom:10px">南向近 1 年分位</div>
    <div style="font-size:56px;font-weight:900;color:var(--red)">{NANXIANG_1Y_RANK:.0f}%</div>
    <div style="font-size:22px;color:var(--muted);margin-top:8px">+130亿 · 日均+42亿</div>
  </div>
</div>
...
<div style="margin-top:auto;padding:18px 24px;background:linear-gradient(90deg,rgba(210,153,29,0.12),rgba(210,153,29,0.03));border-left:5px solid var(--orange);border-radius:8px;margin-bottom:10px">
  <div style="font-size:28px;font-weight:900;color:var(--orange)">⚠️ 散户友情提醒</div>
  <div style="font-size:23px;color:var(--text2);margin-top:8px">港股 ≠ A 股 · 无涨跌幅限制 · 汇率波动风险 · 高位股次日常见 -3% 补跌</div>
</div>
...
<div style="display:flex;align-items:center;gap:24px;padding:26px 28px;background:linear-gradient(135deg,var(--card) 0%,rgba(0,0,0,0.3) 100%);border:2px solid var({col});border-radius:16px;box-shadow:0 4px 20px rgba(0,0,0,0.3)">
  ...
  <div style="flex:1">
    ...
    <div style="font-size:26px;color:var(--muted);margin-top:8px">{body}</div>
  </div>
</div>""" for num, title, col, body in cards)

    body = f"""
<div class="top-pill"><div class="pill" style="background:var(--rose)">关注我</div></div>
<div style="text-align:center;font-size:32px;font-style:italic;color:var(--text2);margin-top:20px">
  港科明天还能续命吗? 数据每天替你盯 📊
</div>
...
<div style="margin-top:32px;padding:28px 32px;background:linear-gradient(135deg,var(--gold) 0%,#e8b73a 100%);border-radius:16px;text-align:center;box-shadow:0 8px 32px rgba(240,192,64,0.35)">
  ...
  <div style="font-size:27px;color:var(--bg);margin-top:10px;opacity:0.8">明早 9:15 继续给你递港科盘前情报</div>
</div>

<div style="margin-top:auto;text-align:center;padding-bottom:12px">
  <div style="font-size:30px;font-weight:900;color:var(--cyan);margin-bottom:10px">💬 评论区告诉我</div>
  <div style="font-size:27px;color:var(--text2);margin-bottom:8px">你是 港科党 还是 A股党? 明天该抄底还是减仓?</div>
  <div style="font-size:23px;color:var(--muted)">明天想看 哪只港股 or ETF 的盘后追踪? 评论区点名 →</div>
</div>
